...
class Background(simbolo: Simbolo, val tipoColor: TipoColor): AtributoEstilo(simbolo) {
    override fun agregarAtributo(
        infoCalculo: InfoCalculo,
        elemento: Elemento
    ) {
        if(elemento is Estilo){
            if(elemento.colorBackCadena.isNotEmpty()){
                agregarMensaje(infoCalculo, simbolo, "atributo repetido")
                return
            }
            elemento.colorBackCadena = tipoColor.cadena(infoCalculo)
            return
        }
